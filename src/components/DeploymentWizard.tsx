...
import { Check, ChevronRight, ChevronLeft, Server, Settings, Users, Network, CheckCircle2, Zap, Clock, AlertCircle, Plus } from 'lucide-react';
...
import { getTemplates, getTemplate, getTemplateVersion, getTemplateVersions, type TemplateDto, type TemplateVersionDto, type TemplateParameter } from '../api/templates';
import { getMyCourses, getCourseGroups, createCourseGroup, getCourseMembers, getGroupMembers, addGroupMembers, type CourseDto, type CourseGroupDto, type CourseMemberDto, type GroupMemberDto } from '../api/courses';
...
export function DeploymentWizard({ templateId, onCancel, onComplete }: DeploymentWizardProps) {
  ...
  const [groupMembersMap, setGroupMembersMap] = useState<Record<string, GroupMemberDto[]>>({});
  const [templateVersions, setTemplateVersions] = useState<TemplateVersionDto[]>([]);
  const [templateVersionData, setTemplateVersionData] = useState<TemplateVersionDto | null>(null);
  const [loading, setLoading] = useState({ template: true, courses: true, version: false, groups: false, members: false });
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        setLoading({ template: true, courses: true, version: false, groups: false, members: false });
        setError(null);
        ...
        setLoading({ template: false, courses: false, version: false, groups: false, members: false });
      } catch (err) {
  ...
  const steps = useMemo(() => {
    ...
    const configSteps = stepOrder
      .filter(step => parametersByStep[step] && parametersByStep[step].length > 0)
      .map((step, index) => ({
        id: index + 1,
  ...
  const handleCreateGroup = async () => {
    ...
    try {
      ...
      setError(null);
      const groupRes = await createCourseGroup(selectedCourseId, newGroupName.trim());
      const newGroup = groupRes.data;
      
      // If members were selected, add them to the group
      if (selectedMemberIds.length > 0) {
        try {
          await addGroupMembers(selectedCourseId, newGroup.id, selectedMemberIds);
        } catch (err) {
  ...
  const handleDeploy = async () => {
    ...
    try {
      ...
      //   group_ids: deploymentMode === 'per_group' ? selectedGroupIds : undefined,
      //   course_member_ids: deploymentMode === 'per_student' ? undefined : undefined, // TODO: implement student selection
  ...
  const renderParameterField = (param: TemplateParameter) => {
        ...
        <div key={param.name} className="space-y-2">
          ...
          >
            <SelectTrigger id={fieldId}>
              <SelectValue placeholder={`Wählen Sie ${param.label || param.name}`} />
                </SelectTrigger>
                <SelectContent>
    ...
    if (param.type === 'boolean') {
      return (
        <div key={param.name} className="flex items-center justify-between p-4 bg-slate-50 rounded-lg">
  ...
  const renderStepContent = () => {
    ...
    if (currentStepData.stepKey === 'template') {
        return (
          ...
          <div className="p-4 bg-slate-50 rounded-lg border border-slate-200">
            ...
            {selectedTemplate?.description && (
              <p className="text-xs text-slate-600 mt-2">{selectedTemplate.description}</p>
            )}
                </div>
              ...
              >
                  ...
                  <SelectValue placeholder={loading.version ? "Lädt..." : templateVersions.length === 0 ? "Keine Versionen verfügbar" : "Version auswählen"} />
                  </SelectTrigger>
                  <SelectContent>
                  {templateVersions.map(version => (
                    <SelectItem key={version.id} value={version.id}>
                      {version.version} {version.is_active && <Badge variant="secondary" className="ml-2">Aktiv</Badge>}
                    </SelectItem>
                  ))}
                  </SelectContent>
                </Select>
            ...
            >
                ...
                <SelectItem value="per_course">Pro Kurs (eine VM für den gesamten Kurs)</SelectItem>
                  </SelectContent>
                </Select>
            <p className="text-xs text-slate-500 mt-2">
              Wählen Sie, wie die Studenten aus dem Kurs aufgeteilt werden sollen
            </p>
            </div>
                ...
                {!selectedCourseId ? (
                  ...
                  <div className="mt-2 p-3 bg-amber-50 border border-amber-200 rounded-lg">
                    <span className="text-xs text-amber-800 block">
                      Der API-Endpoint für Gruppen ist noch nicht implementiert. Bitte verwenden Sie vorerst einen anderen Deployment-Modus.
                    </span>
              </div>
            ...
            <div>
                          <Label htmlFor="new-group-name" className="text-xs">Gruppenname</Label>
                          <Input
                            id="new-group-name"
                            value={newGroupName}
            ...
            <div>
                            <Label className="text-xs mb-2 block">Studenten auswählen (optional)</Label>
        ...
        return (
                          <div key={group.id} className="space-y-2 pb-2 border-b border-slate-100 last:border-0 last:pb-0">
                              ...
                              >
                                ...
                                {groupMembers.length > 0 && (
                                  <span className="text-xs text-slate-500 ml-2">
                                    ({groupMembers.length} Student{groupMembers.length !== 1 ? 'en' : ''})
                                  </span>
                                )}
                              </Label>
                            </div>
                            {groupMembers.length > 0 && (
                              <div className="ml-6 space-y-1">
                                <span className="text-xs text-slate-500">Studenten:</span>
            ...
            <div className="p-6 bg-gradient-to-br from-teal-50 to-blue-50 border border-teal-200 rounded-lg">
              ...
              <div className="space-y-3">
                ...
                <span className="text-sm text-slate-900">
                  {selectedTemplate?.name || '-'} {templateVersionData?.version ? `(${templateVersionData.version})` : ''}
                </span>
              </div>
              <div className="flex justify-between">
                <span className="text-sm text-slate-600">Deployment-Name:</span>
                <span className="text-sm text-slate-900">{deploymentName || '-'}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-sm text-slate-600">Kurs:</span>
                <span className="text-sm text-slate-900">
                  {selectedCourse ? `${selectedCourse.name} (${selectedCourse.semester})` : '-'}
                </span>
                </div>
                <div className="flex justify-between">
                <span className="text-sm text-slate-600">Gruppieren:</span>
                <span className="text-sm text-slate-900">{deploymentModeLabels[deploymentMode] || deploymentMode}</span>
                </div>
              {deploymentMode === 'per_group' && selectedGroupIds.length > 0 && (
                <div className="flex justify-between">
                  <span className="text-sm text-slate-600">Ausgewählte Gruppen:</span>
                  <span className="text-sm text-slate-900">
                    {selectedGroupIds.length} Gruppe(n): {courseGroups
              ...
              <div className="flex justify-between">
                ...
                <span className="text-sm text-slate-900">{runtimeLabels[runtime] || runtime}</span>
            </div>
                </div>
                </div>

          {templateVersionData?.parameters && templateVersionData.parameters.filter(p => !p.hidden).length > 0 && (
            <Card className="border-slate-200">
              <CardHeader>
                <CardTitle className="text-base">Template-Parameter</CardTitle>
              </CardHeader>
                  ...
                  .map(param => {
                    ...
                    return (
                      <div key={param.name} className="flex justify-between text-sm">
                        <span className="text-slate-600">{param.label || param.name}:</span>
